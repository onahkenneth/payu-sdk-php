...
class ResultPrinter
{
    ...
    protected static function printConsoleObject($object, $error = null)
    {
        ...
        if ($object) {
            if (is_a($object, 'PayU\Model\PayUModel')) {
                /** @var $object \PayU\Model\PayUModel */
                echo $object->toJSON(128);
            } elseif (is_string($object) && \PayU\Validation\JsonValidator::validate($object, true)) {
    ...
    {
        $data = null;
        if ($exception instanceof \PayU\Exception\NetworkException) {
